...
void CGame::InitKeyboard(LPKEYEVENTHANDLER handler)
{
	...
	// Set the data format to "keyboard format" - a predefined(xác định trước) data format 
	//
	// A data format specifies(đã xác định rõ) which controls on a device we
	...
	hr = didv->SetCooperativeLevel(hWnd, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE); //set mức độ điều khiển
